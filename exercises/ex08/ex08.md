...
GLib is a bundle of low level system libraries written in C. It provides advanced data structures and implements functions that provide threads. It is written by GNOME and was originally part of GTK+, but was separated so that it can be used for non-GUI uses. Thus, any developer can work with GLib. Yes, it probably would've been better if `glibc` was actually named `GLib` and `GLib` were named something less misleading.

*  What are the most important pros and cons of using GLib, as opposed
to another library, or just using core C features?

GLib provides more complex data structures and containers that are not really in the C language.

*  What do you have to do to compile and run a "Hello GLib" example?

You need the glib runtime and development libraries installed.
